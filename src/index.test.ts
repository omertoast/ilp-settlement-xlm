...
test.concurrent(
  'Sends and receives XLM settlements on testnet',
  async () => {
    ...
    expect(amountSettled).toStrictEqual(new BigNumber(378.1234567)) // Ensure settlement doesn't include amount too precise
...
test.concurrent(
  ...
  async () => {
    let engineA: XlmSettlementEngine
    let engineB: XlmSettlementEngine
    ...
    engineA = await createEngine({ xlmSecret: secretA })(contextA)
    engineB = await createEngine({ xlmSecret: secretB })(contextB)
    ...
    // Ensure a new payment memo was generated for the 2nd request
    expect(paymentsDetails1.paymentMemo).not.toBe(paymentsDetails2.paymentMemo)

    // Ensure the same, correct XLM address was returned for each request
    expect(paymentsDetails1.xlmAddress).toBe(addressB)
    expect(paymentsDetails2.xlmAddress).toBe(addressB)
...
test.todo('Payments without a payment memo are rejected')

test.todo('Non-XLM trustline payments are not credited as incoming settlements')